...
def create_msm22_csvs(input_file):
    """[bold cyan]Create CSV files from MSM22 Excel sheets.[/]
    ...
    This tool converts an MSM22 Excel file into separate CSV files, one for each sheet.
    The output files will be created in the same directory as the input file.

    [bold red]Note:[/] The Info sheet will be skipped automatically.
    """
    try:
        input_path = Path(input_file)
        output_path = (
            input_path.parent
        )  # Output path is the same as input file's directory
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            if "isp_subregion_override" in df.columns:
                df["isp_subregion"] = df["isp_subregion_override"].fillna(
                    method="bfill"
                )
            ...
            for mapped_name, original_name in SHEET_NAME_MAPPING.items():
                ...
                safe_name = "".join(
                    c if c.isalnum() else "_" for c in sheet_name.lower()
                )
                safe_name = safe_name.replace("msm22_", "")
                csv_path = output_path / f"{safe_name}.csv"
                df.to_csv(csv_path, index=False)
                logger.warning(
                    f"No mapping found for sheet '{sheet_name}', using sanitized name: {csv_path}"
                )
